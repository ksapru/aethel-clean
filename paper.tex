...
%% Coordinates are read from scaling_results.dat, emitted by
%% backend/scaling_curve.py (emit_pgfplots_dat). Do not hand-edit.
%% BM25
\addplot[aethelBlue!30, name path=bm25hi, draw=none] table[x=size,y=bm25hi] {scaling_results.dat};
\addplot[aethelBlue!30, name path=bm25lo, draw=none] table[x=size,y=bm25lo] {scaling_results.dat};
\addplot[aethelBlue!25, fill opacity=0.5] fill between[of=bm25hi and bm25lo];
\addplot[aethelBlue, thick, solid, mark=*, mark size=2pt] table[x=size,y=bm25] {scaling_results.dat};
\addlegendentry{BM25}
%% Aethel-NER3
\addplot[aethelpurple!30, name path=ner3hi, draw=none] table[x=size,y=ner3hi] {scaling_results.dat};
\addplot[aethelpurple!30, name path=ner3lo, draw=none] table[x=size,y=ner3lo] {scaling_results.dat};
\addplot[aethelpurple!25, fill opacity=0.5] fill between[of=ner3hi and ner3lo];
\addplot[aethelpurple, thick, dashed, mark=square*, mark size=1.8pt] table[x=size,y=ner3] {scaling_results.dat};
\addlegendentry{Aethel-NER3}
%% MiniLM
\addplot[aethelTeal!30, name path=densehi, draw=none] table[x=size,y=minilmhi] {scaling_results.dat};
\addplot[aethelTeal!30, name path=denselo, draw=none] table[x=size,y=minilmlo] {scaling_results.dat};
\addplot[aethelTeal!25, fill opacity=0.5] fill between[of=densehi and denselo];
\addplot[aethelTeal, thick, dotted, mark=triangle*, mark size=2pt] table[x=size,y=minilm] {scaling_results.dat};
\addlegendentry{Dense (MiniLM)}
%% BGE-base
\addplot[aethelTeal!70!black, thick, dashdotted, mark=diamond*, mark size=2pt] table[x=size,y=bge] {scaling_results.dat};
\addlegendentry{Dense (BGE-base)}
%% GTE-base
\addplot[aethelTeal!55!red, thick, densely dotted, mark=o, mark size=2pt] table[x=size,y=gte] {scaling_results.dat};
\addlegendentry{Dense (GTE-base)}
\end{axis}
\end{tikzpicture}
\caption{Multi-hop HR@5 as corpus size grows from 100 to 4{,}123 chunks, over the 20 multi-hop queries. Gold passages are retained in every subsample, so only distractor density varies. Bands show $\pm 1\sigma$ over 5 resamples (seed 42) for BM25, Aethel-NER3 and MiniLM; the full-corpus point is a single deterministic trial, so all bands close there. Aethel-NER3's band is zero-width at 100 chunks because all five resamples returned exactly 0.800, not because variance is unreported. Coordinates are read from \texttt{scaling\_results.dat}, emitted by \texttt{backend/scaling\_curve.py}, rather than transcribed by hand. E5-base is omitted for legibility (it tracks MiniLM closely: 0.800$\rightarrow$0.450). Every dense encoder degrades far more steeply than BM25 (0.800$\rightarrow$0.700, $-$0.100): MiniLM $-$0.390, E5-base $-$0.350, BGE-base $-$0.310, GTE-base $-$0.500. BGE-base leads all systems below 1{,}000 chunks (0.910 at 100) but converges to Aethel-NER3's 0.600 at full scale; GTE-base collapses hardest despite being the strongest encoder on the closed-pool benchmarks. The full-corpus Aethel-NER3 point (0.600) matches its multi-hop HR@5 in Table~\ref{tab:financial_eval}.}
\label{fig:scaling}
\end{figure}
...
Documents are converted to text and chunked to a target of 800 characters with 150 characters of overlap (\texttt{rag\_service.py}), with sentence-boundary alignment. Table extraction is best-effort; chunks derived from tabular regions retain row structure as delimited text. No deduplication is applied beyond exact-match removal.
